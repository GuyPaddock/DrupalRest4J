...
implements ExclusionStrategy
{
  ...
  public boolean shouldSkipField(FieldAttributes attributes)
  {
    ...
    /* Don't skip fields we've been asked to preserve, as well as fields that
     * don't pertain to the type of object we're working with.
     */
    if (this.fieldsToPreserve.contains(fieldName) || !containerClass.isAssignableFrom(this.updatedObject.getClass()))
    {
      skip = false;
    }
    ...
    else
    {
      ...
      catch (IllegalArgumentException | IllegalAccessException | NoSuchFieldException | SecurityException ex)
      {
        throw new RuntimeException(
          String.format(
            "Failed to retrieve value of existing entity field \"%s.%s\": %s",
